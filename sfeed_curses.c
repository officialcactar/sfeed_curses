...
#define THEME_ITEM_SELECTED()         do { if (p->focused) ttywrite("\x1b[39;44m"); } while(0) /* bright yellow fg, blue bg */
#define THEME_SCROLLBAR_FOCUS()       do { ttywrite("\x1b[39m");    } while(0) /* blue fg */
#define THEME_SCROLLBAR_NORMAL()      do { ttywrite("\x1b[39m");    } while(0)
#define THEME_SCROLLBAR_TICK_FOCUS()  do { ttywrite("\x1b[49m");    } while(0) /* blue bg */
#define THEME_SCROLLBAR_TICK_NORMAL() do { ttywrite("\x1b[49m");    } while(0)
#define THEME_STATUSBAR()             do { attrmode(ATTR_BOLD_ON); ttywrite("\x1b[30;44m"); } while(0)
...
nextpage:
		...
		case 'z':
			...
			if (selpane == PaneFeeds && panes[selpane].nrows) {
				...
				item = (struct item *)row->data;
				markread(p, p->pos, p->pos, 1);
				forkexec((char *[]) { "mpv", item->fields[FieldLink], NULL }, plumberia);
		...
		case 'E': /* yank: pipe TSV field to yank enclosure to clipboard */
			if (selpane == PaneItems && panes[selpane].nrows) {
				...
				item = (struct item *)row->data;
				switch (ch) {
				case 'y': pipeitem(yankercmd, item, FieldLink, yankeria);
				markread(p, p->pos, p->pos, 1);
				break;
